...
/**
 * Class used to scan SpringBoot fat .jar files.
 */
public class SpringBootScanner extends AbstractScanner {
	private static final String CLASSES_LOCATION = "BOOT-INF/classes";

	/**
	 * Scan a SpringBoot .jar archive
	 *
	 * @param jar The file
	 * @return The archive
	 */
	@Override
	public Archive scan(File jar) {
		return scan(jar, null, null, null);
	}

	/**
	 * Scan a SpringBoot fat.jar archive
	 *
	 * @param fatJar      The file
	 ...
	 */
	@Override
	public Archive scan(File fatJar, Map<String, SortedSet<String>> gProvides, List<Profile> known, Set<String> blacklisted) {
		...
		String name = fatJar.getName();

		try {
			String canonicalPath = fatJar.getCanonicalPath();
			warFile = new JarFile(fatJar);
			File extractedDir = fatJar.isFile() ? Extractor.extract(warFile) : fatJar;
			...
			while (warEntries.hasMoreElements()) {
				...
				if (entryName.startsWith(CLASSES_LOCATION) && entryName.endsWith(".class")) {
					try {
			...
			String classesName = name + CLASSES_LOCATION;
			...
			warArchive = new WarArchive(name, classVersion, lManifest, lSign, requires, provides, classDependencies,
	...
	static boolean isBootJar(File file) {
		...
		try {
			...
			while (entries.hasMoreElements()) {
				...
				if (entryName.startsWith(CLASSES_LOCATION)) {
					System.out.println("Spring Boot folder found: " + entryName);
					return true;
				}
			}
		} catch (Exception e) {
